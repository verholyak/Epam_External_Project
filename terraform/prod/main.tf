...
resource "aws_launch_configuration" "web" { 
  ...
  instance_type   = var.instance_type                             // variables
  key_name        = "key_for_ansible"                             // add key pair
...
resource "aws_autoscaling_group" "web" {
  name                 = "ASG-${aws_launch_configuration.web.name}" // поміняється імя в залежності від зміни aws_launch_configuration
  launch_configuration = aws_launch_configuration.web.name
  min_size             = 2     // 1
  max_size             = 2
  min_elb_capacity     = 2     // 1
